...
def p_insert_into(p):
    ...
    p[0] = {"action": "insert_into", "table": p[3], "columns": p[5], "values": p[9]}
...
def p_error(p):
    ...
    if p:
        ...
        syntax_errors.append("Syntax error, the structure of your sentence is invalid.")
...
def run_semantic_analyzer(connection, query):
    ...
    result_string = (
        "Semantic Result: " + ("Valid - " if result else "Invalid - ") + text
    )
    return result_string, error_occurred
